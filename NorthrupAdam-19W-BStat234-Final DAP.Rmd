...
Of the available covariates, only `SINCEHIV`, time since HIV diagnosis, and `Age` are continuous. Variables for gender, sexual orientation, ethnicity, race, and education are coded as binary variables with numerous options that were pared down for the purposes of this analysis. Employment status was not already coded as binary, and so had to be treated differently, as detailed below. Demographic covariates were collapsed to reflect categories for which prior information was available. As the purpose of this analysis is not to assess the effect of demographic variables, reference categories will not be reported. Prior information was taken from Rideout and Fox, 2018, which assessed the effects of age, gender, LGBTQ status, and race. Employment status was not detailed.  
...
### Models
Three models were evaluated and compared to determine the effect of `SINCEHIV`. The first model included `Age`, but since `Age was found to be correlated with `SINCEHIV`, ($\rho = $`r corAgeSinceHIV`), a second model with `Age` was considered. Both of these models included `Site` as a random effect, as participant outcomes were anticipated to be correlated within site. The last model did not include `Age` or random effects. Regression coefficients were expected to follow normal distributions, though these distributions are unknown. They were generally estimated to be likely negative, zero, or positive, with means of -1, 0, or 1 and standard deviation of 1. This allows each distribution to include zero. See Table 2 for prior normal distribution parameters.  
```{r model1}
cat(
"model{
  for (i in 1:N.obs) {
