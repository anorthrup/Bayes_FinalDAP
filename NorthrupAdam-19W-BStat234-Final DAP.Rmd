...
### Models
Three models were evaluated and compared to determine the effect of `SINCEHIV`. The first model included `Age`, but since `Age` was found to be correlated with `SINCEHIV`, ($\rho$ = `r round(corAgeSinceHIV, 2)`), a second model without `Age` was considered. Both of these models included `Site` as a random effect, as participant outcomes were anticipated to be correlated within site. The last model did not include `Age` or random effects. The third model will include `Age` if the first model is shown to outperform the second model without `Age`. Models followed the general form:  
$$
...
## Results  
Models 1 and 2 had high autocorrelation and high spikes in time series plots. Iterations were increased to 5100 with thinning of 5, but because autocorrelation was still relatively high, iterations were increased to 8100 with thinning of 8. Burnin was never increased because models appeared to converge quickly. All three models were run and compared with 8100 iterations. Figure 1 shows autocorrelation and convergence of random effects $\sigma_b$ for all three models. Autocorrelation and convergence for $\alpha$ coefficients were also compared, but only those for Model 2 are shown (Figures 2 and 3). Autocorrelation is lower for Model 3, though convergence is poor. Autocorrelation is slightly higher for Models 1 and 2, but convergence is good. Model 1 convergence has slightly higher spikes, and slightly higher DIC (Table 3), so Model 2 appears to be the best-performing.  Convergence for $\alpha$ in Figure 3 is good for covariates.  

Model 2 was assessed linearity in its continuous variable, `SINCEHIV`. Figure 4 shows `SINCEHIV` plotted against $\pi_i$ and $y_i$ with LOESS smoother for each. The LOESS curve for $\pi_i$ is relatively straight, showing that the model is linear in `SINCEHIV`. The curves are also relatively similar, showing good model fit.  

The log odds ratio for time since diagnosis on whether a participant sought health information online is seen in Table 4 as -0.06. Table 4 also includes the log odds ratios for other covariates, but the effects of these covariates were not of interest in the research question. The estimate for the SD for the random effect distribution is also listed.  
...
```{r jagsPlotsSigma, include = TRUE, fig.cap = "Figure 1. Autocorrelation and Convergence of Sigmas"}
...
```{r jagsPlotsAlpha1, include = TRUE, fig.cap = "Figure 2. Autocorrelation and Convergence of Model 2 Alphas 1-6"}
par(mfrow = c(2, 3))
...
```{r jagsPlotsAlpha2, include = TRUE, fig.cap = "Figure 3. Autocorrelation and Convergence of Model 2 Alphas 7-12"}
par(mfrow = c(3, 2))
...
kable(tibble(Model = c("Model 1", "Model 2", "Model 3"),
             DIC = c(dicMod1, dicMod2, dicMod3)), 
      digits = 1,
...
```{r diagnostics, include = TRUE, fig.cap = "Figure 4. Linear Model Diagnostics"}
...
for (i in 1:length(priorMeans)) {
  priorX[(40435*i - 40434):(40435*i)] <- 
    rnorm(40435, as.numeric(priorMeans[i]), 1)
