...
Of the available covariates, only `SINCEHIV` (time since HIV diagnosis), and `Age` are continuous. Figures 1 and 2 show the distributions for `Age` and `SINCEHIV` broken down by whether participants sought health information. Figure 3 shows the distribution for the log of `SINCEHIV`.  
...
plotSINCEHIVlog <- ggplot(demoPlots, aes(x = log(SINCEHIV), fill = factor(HealthInfo))) +
  ...
  facet_grid(~HealthInfo) +
  labs(y = "Count", title = "Distribution of log of Time Since HIV Diagnosis Stratified by Health Information Seeking")
...
## Results  
Models 1 and 2 had high autocorrelation and high spikes in time series plots. Iterations were increased to 5100 with thinning of 5, and then again to 8100 with thinning of 8. Burnin was never increased because models appeared to converge quickly. The models with 8100 iterations were not noticeably different from those with 5100, and so the models with 5100 were compared. Figure 5 shows autocorrelation and convergence of random effects $\sigma_b$ for all four models. Autocorrelation and convergence for $\alpha$ coefficients were also compared, but only those for Model 4 are shown (Figures 6 and 7). Autocorrelation is lower for Model 3, though convergence is poor, by orders of magnitude. Autocorrelation is similar and slightly higher for Models 1, 2, and 3, but convergence is good. Models 1, 2, and 3 are indistinguishable graphically, but model 4 has the lowest DIC (Table 3), so Model 4 appears to be the best-performing. Convergence for $\alpha$ in Figure 7 is good for all covariates.  

Model 2 and model 4 were assessed for linearity in the continuous variable, `SINCEHIV`. Figure 8 shows `SINCEHIV` plotted against $\pi_i$ and $y_i$ with LOESS smoothers for each. The curves are similar, though they differ at specific values of `SINCEHIV`. It is possible that due to the low number of observations at the high end of the curve, the differences are due to noise. The differences in the middle (near 11 years) may also be due to the upward tail of the LOESS curve on `HealthInfo` at high values. The differences in the two curves at values of `SINCEHIV` under 5 years lend possible evidence against the fit of the model. Generally the probability of seeking health information, $\pi$, is high for lower times since HIV diagnosis, and the same is true for number of participants seeking health information, y. However, $\pi$ is higher than y at 1 year according to the LOESS curve. The slight dip in both curves at 4 years also takes away from the linearity of the model for this covariate. Model 4 was created in order to account for these differences in LOESS curves. Figure 9 shows the same curves for Model 4, but with slightly closer tails on the low end of `SINCEHIV`. These differences should still be noted as a potential limitation of the model.  
...
```{r plotAge, include = TRUE, fig.cap = "Figure 1. Distribution of Participant Age"}
plotAge
```

```{r plotSINCEHIV, include = TRUE, fig.cap = "Figure 2. Distribution of Time Since HIV Diagnosis (SINCEHIV)"}
plotSINCEHIV
```

```{r plotSINCEHIVlog, include = TRUE, fig.cap = "Figure 3. Distribution of log(SINCEHIV)"}
plotSINCEHIVlog
```

```{r plotCorr, include = TRUE, fig.cap = "Figure 4. Scatterplot of Age against SINCEHIV"}
plotCorr
```

```{r jagsPlotsSigma, include = TRUE, fig.cap = "Figure 5. Autocorrelation and Convergence of Sigmas"}
...
```{r jagsPlotsAlpha1, include = TRUE, fig.cap = "Figure 6. Autocorrelation and Convergence of Model 2 Alphas 1-6"}
par(mfrow = c(2, 3))
...
```{r jagsPlotsAlpha2, include = TRUE, fig.cap = "Figure 7. Autocorrelation and Convergence of Model 2 Alphas 7-12"}
par(mfrow = c(3, 2))
...
```{r diagnosticsMod2, include = TRUE, fig.cap = "Figure 8. Model 2 Diagnostics"}
...
```{r diagnosticsMod4, include = TRUE, fig.cap = "Figure 9. Model 4 Diagnostics"}
