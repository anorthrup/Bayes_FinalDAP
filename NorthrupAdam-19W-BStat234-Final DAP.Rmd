...
ggplot(diagMod2, aes(x = xa, y = logitPi)) +
  geom_point() +
  geom_smooth(method = "loess")
ggplot(diagMod2, aes(x = xa, y = pi)) +
  geom_point() +
  geom_smooth(method = "loess", color = "green", se = FALSE) +
  geom_point(aes(y = y), color = "blue") +
  geom_smooth(aes(y = y), method = "loess", color = "blue", se = FALSE) +
  labs(y = "pi", title = "Pi and Y against Time Since HIV Diagnosis",
       x = "Time Since HIV (yr)")
```
alpha[1] ~ dnorm(0, 1)
  alpha[2] ~ dnorm(0, 1)
  ...
  alpha[12] ~ dnorm(0, 1)
```{r plotPosterior}
priorMeans <- c(0, 0, 1, 1, 1, 0, 0, 0, 1, 1, 1, 0)
...
postMod2 <- burnMod2$Burnin.sims.matrix %>%
  ...
  mutate(Key = factor(Key, levels = unique(Key)),
         Prior = priorX)
ggplot(postMod2, aes(x = Posterior)) +
