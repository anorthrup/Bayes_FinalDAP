...
## Conclusions  
Table 4 shows that the 2.5th percentile and 97.5th percentile for the regression coefficient of time since HIV diagnosis (`SINCEHIV`) are both below zero, indicating that there is indeed an effect on health-seeking behavior. This is reinforced by the fact that all estimates are below zero, and thus the probability of the $\alpha$ for `SINCEHIV` being below zero is 1.00. This indicates that for each increase in year since HIV diagnosis, the odds of seeking health information online is reduced by `r round(1 - exp(-.40), 2)`. Figure 7 shows the prior, likelihood, and posterior distributions, together and separated due to the extreme differences in scale.  
